...
$w=strtotime("-1 week");
$previous_week= date("Y-m-d 00:00:00", $w) . "<br>";
...
$query = "SELECT * FROM product_order where product_order.product_order_time
 Between '$previous_week' AND '$current_time' " ;
...
$currentPage = $_GET['page'];
  ...
  //$page=1;
  header("location:weekly_order.php?page=0");
}
else
$currentPage=1;
...
$result="SELECT customer.c_name,customer.c_email,customer.c_contact_no,customer.c_zip_code,customer.c_address,orders.p_name,orders.details,product_order.quantity,orders.price, 
    ...
    where product_order.product_order_time  between '$previous_week' AND '$current_time' 
    order by time DESC  
...
if($result1->num_rows>0)
{
	...
	{	
		...
		<td>
		<a class='btn btn-danger' href=?wo_id=".$row['product_order_id'].">Delete</a>
		</td>
		</tr>";
